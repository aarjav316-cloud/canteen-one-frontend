...
import { ArrowLeft, Clock, CheckCircle2, XCircle, ChefHat, PackageCheck, ReceiptText, QrCode } from "lucide-react";
...
const STATUS_CONFIG = {
  pending:   { label: "Accepted",  color: "text-amber-600",  bg: "bg-amber-50",   border: "border-amber-200",  dot: "bg-amber-400",  icon: Clock        },
  paid:      { label: "Accepted",  color: "text-blue-600",   bg: "bg-blue-50",    border: "border-blue-200",   dot: "bg-blue-400",   icon: CheckCircle2 },
  preparing: { label: "Preparing", color: "text-orange-600", bg: "bg-orange-50",  border: "border-orange-200", dot: "bg-orange-400", icon: ChefHat      },
  ready:     { label: "Ready",     color: "text-green-600",  bg: "bg-green-50",   border: "border-green-200",  dot: "bg-green-500",  icon: PackageCheck },
  completed: { label: "Completed", color: "text-gray-500",   bg: "bg-gray-50",    border: "border-gray-200",   dot: "bg-gray-300",   icon: ReceiptText  },
...
const STEP_STATUS = { pending: 0, paid: 0, preparing: 1, ready: 2 };
function OrderCard({ order, cfg, isReady, isActive, currentStep }) {
  const [isExpanded, setIsExpanded] = useState(false);
  const Icon = cfg.icon;
  return (
    <div className={`rounded-[1.25rem] bg-white overflow-hidden shadow-sm border ${isActive ? "border-black/8" : "border-black/5"}`}>
      ...
      <div className="p-4">
        ...
        <div className="flex items-start justify-between">
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-2">
              ...
              <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[9px] font-black uppercase tracking-wider ${cfg.color} ${cfg.bg}`}>
          ...
          <div className="flex flex-col items-end gap-0.5">
            <span className="text-sm font-black text-black">{formatINR(order.totalAmount)}</span>
            <span className="text-[9px] text-gray-300 font-medium capitalize">{order.paymentMethod || "wallet"}</span>
          </div>
        </div>
              ...
              return (
                <div key={step} className="flex items-center flex-1 last:flex-none">
                  <div className="flex flex-col items-center gap-1">
                    ...
                    <span className={`text-[8px] font-bold whitespace-nowrap ${done ? "text-black" : "text-gray-300"}`}>
                      {step}
                    </span>
                  </div>
                  {i < STEPS.length - 1 && (
                    <div className={`flex-1 h-[1.5px] mx-1 mb-4 rounded-full ${i < currentStep ? "bg-[#1A1A1A]" : "bg-gray-100"}`} />
                  )}
                </div>
              );
            })}
        ...
        {isReady && (
          <div className="mt-3 rounded-[0.875rem] bg-green-50 border border-green-100 px-4 py-3 flex items-center justify-between">
            <div>
              <p className="text-[8px] font-black uppercase tracking-[0.15em] text-green-600/60 mb-1">Pickup Code</p>
        ...
        <div className="mt-3 pt-3 border-t border-gray-50 flex items-center justify-between">
          <div className="flex gap-1.5">
            {(order.items || []).slice(0, 3).map((item, i) => (
              <span key={i} className="rounded-full bg-gray-50 px-2 py-0.5 text-[9px] font-semibold text-gray-500 border border-gray-100 truncate max-w-[80px]">
                {item.name}
              </span>
            ))}
              ...
              <div className="mt-3 space-y-1.5">
                {(order.items || []).map((item, i) => (
                  <div key={i} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-black text-gray-300 w-5">{item.quantity}×</span>
                      <span className="text-[11px] font-semibold text-gray-600">{item.name}</span>
                    </div>
                    <span className="text-[11px] font-bold text-gray-800">{formatINR(item.price * item.quantity)}</span>
                  </div>
                ))}
                <div className="flex items-center justify-between pt-2 border-t border-gray-50 mt-2">
                  <span className="text-[10px] font-black text-gray-400 uppercase tracking-wider">Total</span>
                  <span className="text-[12px] font-black text-black">{formatINR(order.totalAmount)}</span>
                </div>
              </div>
            </motion.div>
          )}
...
export default function OrdersPage() {
  ...
  const activeOrders = orders.filter((o) => ["pending", "paid", "preparing", "ready"].includes(o.status));
    ...
    <div className="min-h-screen bg-[#F5F5F5] pb-24 font-sans">
      ...
      <div className="sticky top-0 z-30 bg-[#F5F5F5] relative">
        <div className="px-4 pt-6 pb-3 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-display font-black text-cocoa-900 tracking-tight leading-tight">My Orders</h1>
            ...
            {["Active", "Past"].map((tab) => (
              <button
                ...
                className={`flex flex-1 items-center justify-center gap-1.5 rounded-[0.5rem] py-2.5 text-xs font-black tracking-wide transition-all ${
                  selectedTab === tab ? "bg-[#1A1A1A] text-white shadow-sm" : "text-cocoa-900/40"
                }`}
              >
                {tab}
        ...
        {displayOrders.length > 0 ? (
          <>
            {displayOrders.map((order) => {
              ...
              const isActive = ["pending", "paid", "preparing", "ready"].includes(order.status);
            ...
            {selectedTab === "Past" && (
              <div className="flex items-center gap-4 py-4">
                ...
                <p className="text-[9px] font-black uppercase tracking-[0.2em] text-cocoa-900/20 whitespace-nowrap">No more past orders</p>
