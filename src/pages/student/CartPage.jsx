...
export default function CartPage() {
  ...
  const handleSendOtp = async (e) => {
    e.preventDefault();
    if (!loginForm.mobile || loginForm.mobile.length !== 10) {
    ...
    setOtpLoading(true);
    try {
      await api.post("/auth/send-otp", { mobile: loginForm.mobile });
      setOtpSent(true);
    } catch (err) {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      const { data } = await api.post("/auth/verify-otp", loginForm);
      login(data.token);
      setShowAuthModal(false);
      setLoginForm({ mobile: "", otp: "" });
      setOtpSent(false);
    } catch (err) {
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    try {
      ...
      const registeredMobile = registerForm.mobile;
      setRegisterSuccess(true);
      setTimeout(() => {
        setRegisterSuccess(false);
        setAuthTab("signin");
        setLoginForm({ mobile: registeredMobile, otp: "" });
        setRegisterForm({ name: "", mobile: "", email: "" });
      }, 1500);
    } catch (err) {
  ...
  const payDirectWithRazorpay = async () => {
    setPlacing(true);
    setPayError("");
    try {
      const payload = {
  ...
  const payAtCounter = async () => {
    if (user?.role === "student") {
      setPayError("Students must use Wallet or Online Payment (Prepaid Only).");
      return;
    }
    setPlacing(true);
    setPayError("");
      ...
      {cart.length > 0 ? (
        <>
          ...
          <div className="mx-4 mt-4 rounded-[1.5rem] bg-white p-4 shadow-sm">
            <p className="text-[9px] font-display font-black uppercase tracking-[0.2em] text-cocoa-900/40">
              Order Summary
            </p>
            <div className="mt-3 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-cocoa-900/50">
                  Items ({totalItems})
                </span>
            ...
            <div className="mt-3 flex items-center justify-between">
              ...
              <span className="text-sm font-black text-cocoa-900">
                {formatINR(total)}
              </span>
            </div>
          </div>
              ...
              <AnimatePresence mode="wait">
                {authTab === "signin" ? (
                  <motion.form
                    ...
                    onSubmit={otpSent ? handleLoginSubmit : handleSendOtp}
                    className="space-y-3"
                  >
                    <input
                      type="tel"
                      ...
                      disabled={otpSent}
                      className="w-full rounded-[1rem] bg-[#F5F5F5] px-4 py-3.5 text-sm font-semibold text-cocoa-900 outline-none focus:ring-2 focus:ring-cocoa-900/10 transition-all placeholder:text-cocoa-900/30 disabled:opacity-60"
                    />
                    {otpSent && (
                      <>
                        <input
                        ...
                        <button
                          ...
                          className="text-[11px] font-bold text-cocoa-900/50 hover:text-cocoa-900 transition"
                        >
                  ...
                  >
                    ...
                    {registerSuccess && (
                      <p className="text-[11px] font-bold text-green-500 text-center">
            ...
            >
              ...
              {payError && (
                <div className="mb-3 rounded-xl bg-rose-50 px-3 py-2.5 text-[11px] font-semibold text-rose-500 border border-rose-100 flex items-center gap-2">
                  <span className="flex h-4 w-4 items-center justify-center rounded-full bg-rose-500 text-white text-[9px] flex-shrink-0 font-black">
                    !
                  </span>
                  {payError}
                </div>
              ...
              <div className="space-y-2.5">
                ...
                <button
                  onClick={payDirectWithRazorpay}
                  disabled={placing}
                  className="w-full rounded-[1.25rem] bg-[#1A1A1A] px-4 py-3.5 transition-all active:scale-[0.98] disabled:opacity-60"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-blue-500/20">
                        {/* Simple Razorpay-like icon (Diamond/Card) */}
                        <svg
                          width="16"
                          height="16"
                          viewBox="0 0 24 24"
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-[9px] text-white/40 mt-0.5 tracking-tight">
                          UPI · Cards · Netbanking
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <span className="rounded bg-blue-500/10 px-1.5 py-0.5 text-[8px] font-black uppercase text-blue-400">
                        RAZORPAY
                      </span>
                      <ArrowRight
                        size={14}
                ...
                <button
                  ...
                  disabled={placing || loadingBalance}
                  className="w-full rounded-[1.25rem] border border-gray-100 bg-white px-4 py-3.5 transition-all active:scale-[0.98] disabled:opacity-60 disabled:active:scale-100"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-cocoa-900/5">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="15"
                          height="15"
                      ...
                      <div className="text-left">
                        ...
                        {loadingBalance ? (
                          <div className="mt-0.5 h-3 w-16 bg-gray-100 animate-pulse rounded" />
                        ) : (
                          <p
                            className={`text-[10px] font-semibold mt-0.5 ${walletBalance !== null && walletBalance < total ? "text-rose-400" : "text-cocoa-900/40"}`}
                          >
                ...
                {user?.role !== "student" && (
                  <>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#088366] flex-shrink-0">
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              width="18"
                              height="18"
