...
const DUMMY_ITEMS = [
  { _id: "d1", name: "Masala Dosa", category: "breakfast", description: "Crispy dosa with spicy potato filling", price: 60 },
  { _id: "d2", name: "Veg Burger", category: "snacks", description: "Juicy veggie patty with fresh veggies", price: 80 },
  { _id: "d3", name: "Paneer Rice", category: "meals", description: "Fragrant basmati rice with paneer", price: 120 },
  { _id: "d4", name: "Masala Chai", category: "beverages", description: "Hot spiced Indian tea", price: 20 },
  { _id: "d5", name: "Veg Maggi", category: "snacks", description: "Classic instant noodles with veggies", price: 40 },
  { _id: "d6", name: "Samosa (2 pcs)", category: "snacks", description: "Crispy fried pastry with spiced filling", price: 30 },
  { _id: "d7", name: "Idli Sambhar", category: "breakfast", description: "Soft steamed rice cakes with lentil soup", price: 50 },
  { _id: "d8", name: "Cold Coffee", category: "beverages", description: "Chilled blended coffee with milk", price: 60 },
  { _id: "d9", name: "Dal Rice", category: "meals", description: "Comforting lentil curry with steamed rice", price: 90 },
  { _id: "d10", name: "Veg Sandwich", category: "snacks", description: "Toasted sandwich with fresh vegetables", price: 50 },
  { _id: "d11", name: "Aloo Paratha", category: "breakfast", description: "Stuffed flatbread with spiced potato", price: 55 },
  { _id: "d12", name: "Mango Lassi", category: "beverages", description: "Thick yogurt drink with mango pulp", price: 45 },
];
export default function StudentHomePage() {
  ...
  const [activeCollegesList, setActiveCollegesList] = useState([]);
        ...
        try {
           ...
           if (settingsRes.data.success) {
               ...
               if(settingsRes.data.data.collegeName) {
                   ...
                   setActiveCollegesList(prev => Array.from(new Set([settingsRes.data.data.collegeName, ...prev])));
               }
           }
  ...
  useEffect(() => {
    ...
    const onStatusUpdate = (payload) => {
      setOrders((prev) => prev.map((order) => (order._id === payload.orderId ? { ...order, status: payload.status, pickupCodePlain: payload.pickupCodePlain || order.pickupCodePlain } : order)));
      ...
      setNotifications(prev => [{ time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), message: msg, title: payload.status === "ready" ? "Ready for Pickup" : "Order Update", type: "order" }, ...prev]);
      setToastMessage(msg);
    };
  ...
  const availableColleges = Array.from(new Set([...activeCollegesList].filter(Boolean)));
  const filteredColleges = availableColleges.filter(c => c.toLowerCase().includes(collegeSearch.toLowerCase()));
  const availableCategories = useMemo(() => {
    const cats = new Set(menuItems.map(item => titleCase(item.category)));
    return ["All", ...Array.from(cats)];
  }, [menuItems]);
  useEffect(() => {
    if (isLocationModalOpen || isNotificationModalOpen || isProfileModalOpen || isLoginModalOpen) {
  ...
  }, [isLocationModalOpen, isNotificationModalOpen, isProfileModalOpen, isLoginModalOpen]);
    ...
    <div className="relative min-h-screen pb-24 font-sans bg-[#F5F5F5]">
      ...
      <AnimatePresence>
         {toastMessage && !(activeOrder && barVisible) && (
           <motion.div
             ...
             style={{ position: "fixed", bottom: 24, left: 0, right: 0, zIndex: 200, display: "flex", justifyContent: "center", pointerEvents: "none" }}
           >
             <div style={{ pointerEvents: "auto" }} className="w-auto max-w-[90%] sm:max-w-sm">
               <div className="relative overflow-hidden rounded-full bg-[#1A1A1A]/95 backdrop-blur-xl px-5 py-3 text-white shadow-[0_8px_30px_rgba(0,0,0,0.25)] flex items-center gap-3">
                 ...
                 <p className="text-[11px] font-semibold leading-tight">{toastMessage}</p>
                 <button onClick={() => setToastMessage(null)} className="ml-1 flex-shrink-0 text-white/30 hover:text-white/60 transition active:scale-90">
                   <X size={13} />
                 </button>
      ...
      <div className="relative px-3 pt-4 z-20">
        <header className="flex items-center justify-between">
          ...
          >
            <div className="flex items-center justify-center bg-transparent text-white transition active:scale-90">
              ...
              {loading ? (
                ...
                <>
                  <div className="flex items-center gap-1">
                    ...
                    <ChevronDown 
                      size={14} 
                      className={`text-white drop-shadow-lg transition-transform duration-300 ${isLocationModalOpen ? 'rotate-180' : ''}`} 
                    />
                  </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button 
              onClick={() => { if(user){ setIsProfileModalOpen(true); } else { setIsLoginModalOpen(true); } }}
            ...
            >
              {user ? (
                 user.dp ? (
                     <img src={user.dp} className="h-full w-full object-cover" alt="Profile" />
                 ) : (
                     <span className="font-black text-xs uppercase">{user?.name?.charAt(0) || "U"}</span>
                 )
              ) : (
                 <User size={15} />
              )}
        ...
        <div className="mt-4">
          <div className="relative flex items-center group" onClick={() => navigate("/student/menu?focus=1")}>
      ...
      <section className="mt-5 px-3 space-y-2.5">
        ...
        >
          <div className="relative z-10 flex items-center gap-4">
            ...
            <div>
              <h3 className="font-display text-sm font-black text-cocoa-900 leading-none">Explore Menu</h3>
              <p className="mt-0.5 text-[11px] font-semibold text-cocoa-900/40">Breakfast to beverages</p>
            </div>
          </div>
          <ArrowRight size={16} strokeWidth={2.5} className="text-cocoa-900/20 flex-shrink-0" />
        </Link>
        ...
        <div className="grid grid-cols-2 gap-2.5">
          ...
          >
            <div>
              <h3 className="font-display text-xs font-black text-cocoa-900 leading-none">Open Cart</h3>
              <p className="mt-0.5 text-[10px] font-semibold text-cocoa-900/40">Checkout items</p>
            </div>
          ...
          <div
            onClick={() => user ? navigate("/student/orders") : (setIsLoginModalOpen(true), setAuthTab("signin"))}
          ...
          >
            <div>
              <h3 className="font-display text-xs font-black text-cocoa-900 leading-none">Order History</h3>
              <p className="mt-0.5 text-[10px] font-semibold text-cocoa-900/40">Past orders</p>
            </div>
      ...
      <section className="mt-10 px-3">
        <div className="flex items-center gap-4">
           ...
           <h2 className="font-display text-base font-black text-cocoa-900 whitespace-nowrap tracking-tight uppercase">Fresh from Kitchen</h2>
        ...
        <div className="mt-2 grid gap-3">
          {loading ? (
             <>
               {[1, 2, 3].map(i => (
                 <div key={i} className="flex h-[110px] items-center gap-4 rounded-[1.5rem] bg-white p-3 shadow-sm border border-black/5">
      ...
      {activeOrder && (
        <motion.div
          ...
          style={{ boxShadow: "0 8px 40px rgba(0,0,0,0.18), 0 2px 12px rgba(0,0,0,0.10)" }}
        >
            <div className="flex items-center gap-3 min-w-0">
               ...
               <div className="flex flex-col min-w-0">
                  <p className="text-[11px] font-black uppercase tracking-widest text-[#1A1A1A] leading-none mb-0.5">
                    {activeOrder.status === 'ready' ? 'Ready for Pickup' : 'Now Preparing'}
                  </p>
                  <p className="text-[9px] font-semibold text-cocoa-900/30 truncate">Order #{activeOrder._id.slice(-4).toUpperCase()}</p>
               </div>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
               {activeOrder.status === 'ready' ? (
                 <>
                   ...
                   <Link to="/student/orders" className="flex h-8 w-8 items-center justify-center rounded-xl bg-[#F5F5F5] text-[#1A1A1A]">
                     <ArrowRight size={14} strokeWidth={3} />
                   </Link>
                 </>
               ) : (
                 <Link to="/student/orders" className="flex items-center gap-1.5 rounded-xl bg-[#1A1A1A] px-3 py-1.5 text-white">
                   <span className="text-[10px] font-black uppercase tracking-wider">Track</span>
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between px-2">
                <h2 className="font-display text-2xl font-black text-cocoa-900 tracking-tight">Select College</h2>
                <button 
                  onClick={() => setIsLocationModalOpen(false)}
              ...
              <div className="mt-4">
                <div className="relative flex items-center">
                  <Search size={16} className="absolute left-4 text-cocoa-900/20" />
                  <input 
                    type="text"
                    placeholder="Search your college..."
                    className="w-full rounded-[1.5rem] border-none bg-[#F9FAFB] py-3.5 pl-12 pr-4 text-xs font-bold text-cocoa-900 outline-none focus:ring-4 focus:ring-rose-500/5 transition-all"
                    value={collegeSearch}
              ...
              <div className="mt-5 relative">
                ...
                <div className="flex flex-col gap-2 h-[50vh] overflow-y-auto no-scrollbar pb-4 px-1 pt-2">
                  {filteredColleges.length > 0 ? (
                    filteredColleges.map((college) => (
                      <button
                        key={college}
                        onClick={() => { setSelectedCollege(college); setIsLocationModalOpen(false); }}
                        className={`flex items-center justify-between rounded-[1.5rem] p-4 transition-all active:scale-[0.98] ${
                          selectedCollege === college 
                            ? "border-2 border-green-500 bg-green-50/20"
                            : "border-2 border-[#F9FAFB] bg-[#F9FAFB] hover:border-sand-100"
                        }`}
                      >
                        <span className={`text-xs font-black ${selectedCollege === college ? "text-green-700" : "text-cocoa-900"}`}>
                          {college}
                        </span>
                        {selectedCollege === college && (
                          <div className="flex h-5 w-5 items-center justify-center rounded-full bg-green-500 text-white shadow-lg shadow-green-500/20">
                            <CircleCheck size={12} strokeWidth={4} />
                          </div>
                        )}
                      </button>
                    ))
                    ...
                    <div className="flex items-center justify-center h-full">
                      <p className="text-sm font-bold text-cocoa-900/30 uppercase tracking-wide">Nothing found</p>
                    </div>
                  )}
                </div>
              </div>
      ...
      <AnimatePresence>
        {isNotificationModalOpen && (
          <>
            ...
            >
              <div className="flex items-center justify-between px-2">
                <h2 className="font-display text-2xl font-black text-cocoa-900 tracking-tight">Notifications</h2>
                <button 
                  onClick={() => setIsNotificationModalOpen(false)}
                  className="flex h-9 w-9 items-center justify-center rounded-full bg-[#F9FAFB] text-cocoa-900 transition active:scale-90"
                >
              ...
              {!user ? (
                ...
                <div className="mt-6 flex flex-col items-center text-center gap-4 py-8 px-2">
                  ...
                  <div>
                    <h3 className="font-display text-base font-black text-cocoa-900 leading-tight">Sign in to get notified</h3>
                    <p className="mt-1.5 text-xs font-medium text-cocoa-900/50 leading-relaxed max-w-[240px] mx-auto">
                      Order from {canteenName}, skip the queue and get real-time pickup alerts.
                    </p>
                  </div>
                  <div className="w-full space-y-2 mt-1">
                    <div className="flex items-center gap-3 rounded-[1.25rem] bg-[#F5F5F5] px-4 py-3">
                      <span className="text-lg">🍱</span>
                      <p className="text-xs font-semibold text-cocoa-900/60 text-left">Order ahead &amp; skip the canteen queue</p>
                    </div>
                    <div className="flex items-center gap-3 rounded-[1.25rem] bg-[#F5F5F5] px-4 py-3">
                      <span className="text-lg">📍</span>
                      <p className="text-xs font-semibold text-cocoa-900/60 text-left">Pick up from {canteenName}</p>
                    </div>
                    <div className="flex items-center gap-3 rounded-[1.25rem] bg-[#F5F5F5] px-4 py-3">
                      <span className="text-lg">⚡</span>
                      <p className="text-xs font-semibold text-cocoa-900/60 text-left">Live status — Accepted → Preparing → Ready</p>
                    </div>
                  </div>
                  <button
                    onClick={() => { setIsNotificationModalOpen(false); setIsLoginModalOpen(true); setAuthTab("signin"); }}
                    className="w-full rounded-[1rem] bg-[#1A1A1A] py-3.5 text-sm font-black text-white transition active:scale-[0.98]"
                  >
                    Sign In to Continue
                  </button>
                  <button
                    onClick={() => { setIsNotificationModalOpen(false); setIsLoginModalOpen(true); setAuthTab("signup"); }}
                    ...
                    <div className="flex flex-col gap-3 h-[50vh] overflow-y-auto no-scrollbar pb-4 px-1 pt-2">
                      {notifications.length > 0 ? (
                        notifications.map((notification, index) => (
                          ...
                          >
                            <div className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-white ${notification.type === 'wallet' ? 'text-green-600' : 'text-cocoa-900/40'}`}>
                              {notification.type === 'wallet' ? <CircleCheck size={20} /> : <Clock size={20} />}
                            </div>
                            <div className="flex-1">
                              <div className="flex items-start justify-between">
                                <h3 className={`text-xs font-black ${notification.type === 'wallet' ? 'text-green-700' : 'text-cocoa-900'}`}>{notification.title}</h3>
                                <span className="text-[10px] font-bold text-cocoa-900/40">{notification.time}</span>
                              </div>
                              <p className="mt-1 text-[11px] font-medium text-cocoa-900/60">{notification.message}</p>
                            </div>
                          </div>
                        ))
                      ) : (
                        <div className="flex flex-col items-center justify-center h-full gap-3">
                          ...
                          <p className="text-sm font-bold text-cocoa-900/30 uppercase tracking-wide">No notifications found</p>
                        </div>
                      )}
                    </div>
                  </div>
      ...
      <AnimatePresence>
        {isProfileModalOpen && (
          <>
            ...
            >
               <div className="flex items-center justify-between px-2">
                <h2 className="font-display text-2xl font-black text-cocoa-900 tracking-tight">Your Profile</h2>
                <button 
                  onClick={() => setIsProfileModalOpen(false)}
                 ...
                 <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                       ...
                       <div className="flex flex-col gap-0.5">
                          <h3 className="text-sm font-black text-cocoa-900 tracking-tight leading-none truncate max-w-[150px]">{user?.name || "User"}</h3>
                          <p className="text-[10px] font-bold text-cocoa-900/40 truncate max-w-[150px] tracking-tight flex items-center gap-1"><GraduationCap size={9} /> {user?.college ? user.college.split(" ").slice(0,2).join(" ") : "No College"}</p>
                          <p className="text-[10px] font-bold text-cocoa-900/40 truncate max-w-[150px] tracking-tight flex items-center gap-1"><Phone size={9} /> {user?.mobile || "No Mobile"}</p>
                       </div>
                    </div>
                    <button 
                      onClick={() => { navigate("/student/profile"); setIsProfileModalOpen(false); }}
              ...
              <div className="mt-3 space-y-2">
                 <button 
                  onClick={() => { navigate("/student/orders"); setIsProfileModalOpen(false); }}
                ...
                >
                   <div className="flex items-center gap-2.5">
                      ...
                      <span className="text-xs font-black text-cocoa-900">My Orders</span>
                   </div>
                 ...
                 >
                    ...
                    <span className="text-xs font-black tracking-tight">Sign Out</span>
                 </button>
              </div>
            </motion.div>
          </>
          ...
          <>
            ...
            >
              ...
              <div className="flex items-center justify-between mb-5">
                ...
                <button onClick={closeAuthModal} className="flex h-9 w-9 items-center justify-center rounded-full bg-[#F5F5F5] text-cocoa-900 transition active:scale-90">
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="flex gap-2 mb-5 bg-[#F5F5F5] rounded-[1rem] p-1">
                ...
                >Sign In</button>
                <button
                  onClick={() => { setAuthTab("signup"); setRegisterError(""); }}
                ...
                {authTab === "signin" ? (
                    ...
                    <AnimatePresence>
                      {loginError && (
                        <motion.p initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
                          className="text-[11px] font-bold text-rose-500 text-center">{loginError}</motion.p>
                      )}
                    </AnimatePresence>
                  ...
                  >
                    ...
                    <AnimatePresence>
                      {registerError && (
                        <motion.p initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
                          className="text-[11px] font-bold text-rose-500 text-center">{registerError}</motion.p>
                      )}
                      {registerSuccess && (
                        <motion.p initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
                          className="text-[11px] font-bold text-green-500 text-center">✓ Account created! Switching to sign in...</motion.p>
                      )}
                    </AnimatePresence>
