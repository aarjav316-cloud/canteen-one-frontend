...
import { ArrowLeft, Plus, Wallet, ArrowDownLeft, ArrowUpRight, ShoppingBag, TrendingUp } from "lucide-react";
...
const formatDate = (dateStr) => {
  ...
  if (isToday(dateStr)) return `Today, ${d.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`;
...
export default function WalletPage() {
    ...
    try {
      ...
      if (data.success) {
        ...
        setTransactions(prev => [{
          ...
          label: data.transaction.description,
  ...
  if (loading) {
    return <div className="min-h-screen bg-[#F5F5F5] pb-24 font-sans flex items-center justify-center font-bold text-cocoa-900/40">Loading Database...</div>;
  }
  return (
    <div className="min-h-screen bg-[#F5F5F5] pb-24 font-sans">
      ...
      <div className="px-4 pt-6 pb-3 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-display font-black text-cocoa-900 tracking-tight leading-tight">Payments & Wallet</h1>
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white/50">Canteen Wallet</span>
              </div>
              <AnimatePresence>
                {successAnim && (
                  <motion.div
            ...
            <div className="mt-5">
              <p className="text-[10px] font-bold uppercase tracking-widest text-white/30">Available Balance</p>
              <motion.p
                key={balance}
            ...
            <div className="mt-6 flex items-center justify-between">
              <div className="flex gap-4">
                <div>
                  <p className="text-[9px] font-bold uppercase tracking-widest text-white/30">Total Added</p>
                  <p className="mt-0.5 text-sm font-black text-green-400">{formatINR(totalAdded)}</p>
                </div>
                <div className="w-[1px] bg-white/10" />
                <div>
                  <p className="text-[9px] font-bold uppercase tracking-widest text-white/30">Total Spent</p>
                  <p className="mt-0.5 text-sm font-black text-rose-400">{formatINR(totalSpent)}</p>
                </div>
              </div>
              <button
        ...
        <div className="rounded-[1.5rem] bg-white p-4 shadow-sm">
          <div className="flex items-center gap-2">
            <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-green-50">
              <ArrowDownLeft size={15} className="text-green-500" strokeWidth={2.5} />
            </div>
            <span className="text-[9px] font-black uppercase tracking-widest text-cocoa-900/40">Credits</span>
          </div>
          <p className="mt-2 text-lg font-display font-black text-cocoa-900">{formatINR(totalAdded)}</p>
          <p className="text-[9px] font-bold text-cocoa-900/30">{transactions.filter(t => t.type === "credit").length} transactions</p>
        </div>
        <div className="rounded-[1.5rem] bg-white p-4 shadow-sm">
          <div className="flex items-center gap-2">
            <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-rose-50">
              <ArrowUpRight size={15} className="text-rose-500" strokeWidth={2.5} />
            </div>
            <span className="text-[9px] font-black uppercase tracking-widest text-cocoa-900/40">Debits</span>
          </div>
          <p className="mt-2 text-lg font-display font-black text-cocoa-900">{formatINR(totalSpent)}</p>
          <p className="text-[9px] font-bold text-cocoa-900/30">{transactions.filter(t => t.type === "debit").length} transactions</p>
        </div>
      </div>
      
      <div className="mx-4 mt-5">
        <div className="flex items-center justify-between mb-3">
          <p className="text-[10px] font-display font-black uppercase tracking-[0.18em] text-cocoa-900/40">Transactions</p>
          <span className="text-[9px] font-bold text-cocoa-900/30">{filteredTx.length} records</span>
        </div>
        ...
        <div className="grid gap-2">
          {filteredTx.length > 0 ? (
            filteredTx.map((tx) => (
              ...
              >
                <div className={`flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-[1rem] ${
                  tx.type === "credit" ? "bg-green-50" : "bg-rose-50"
                }`}>
                  {tx.type === "credit"
                    ? <ArrowDownLeft size={18} className="text-green-500" strokeWidth={2.5} />
                    : <ShoppingBag size={18} className="text-rose-500" strokeWidth={2.5} />
                  }
                </div>
                <div className="flex flex-1 flex-col min-w-0">
                  <p className="text-xs font-black text-cocoa-900 leading-tight truncate">{tx.label}</p>
                  <p className="mt-0.5 text-[10px] font-medium text-cocoa-900/40 truncate">{tx.note}</p>
                  <p className="mt-0.5 text-[9px] font-bold text-cocoa-900/25">{formatDate(tx.date)}</p>
                </div>
                <div className="flex flex-col items-end flex-shrink-0">
                  <span className={`text-sm font-black ${tx.type === "credit" ? "text-green-500" : "text-rose-500"}`}>
                    {tx.type === "credit" ? "+" : "−"}{formatINR(tx.amount)}
                  </span>
                  <span className={`mt-0.5 rounded-full px-2 py-0.5 text-[8px] font-black uppercase tracking-wide ${
                    tx.type === "credit"
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              ...
              <p className="text-sm font-black text-cocoa-900/30 uppercase tracking-wide">No Transactions</p>
              <p className="text-xs text-cocoa-900/20">Nothing here for this filter</p>
            </div>
          )}
        </div>
      </div>
        ...
        {showAddMoney && (
          <>
            ...
            >
              ...
              <h2 className="font-display text-xl font-black text-cocoa-900 tracking-tight">Add Money</h2>
              <p className="mt-0.5 text-xs font-medium text-cocoa-900/40">Choose an amount or enter custom</p>
              ...
              <div className="mt-3 relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-sm font-black text-cocoa-900/40">₹</span>
                <input
                  type="number"
              ...
              >
                <span>{adding ? "Adding..." : `Add ${amountToAdd > 0 ? formatINR(amountToAdd) : "Money"}`}</span>
