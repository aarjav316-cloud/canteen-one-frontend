...
export default function AdminProfilePage() {
  ...
  const [canteenName, setCanteenName] = useState(
    () => localStorage.getItem("canteenName") || "Admin Canteen",
  );
  const [canteenDP, setCanteenDP] = useState(
    () => localStorage.getItem("canteenDP") || null,
  );
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (data && data.data) {
          ...
          if (data.data.canteenDP)
            localStorage.setItem("canteenDP", data.data.canteenDP);
        }
      } catch (err) {
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    try {
      await api.patch("/settings", { canteenName });
    ...
    <div className="min-h-screen bg-[#F5F5F5] pb-24 font-sans text-cocoa-900">
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-sand-200 px-4 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link
            to="/admin"
            className="flex h-10 w-10 items-center justify-center rounded-full bg-sand-100 hover:bg-sand-200 transition-colors active:scale-95"
          >
            <ArrowLeft size={20} />
          </Link>
          <div>
            ...
            {editMode ? (
              ...
              >
                <div className="space-y-4">
                  <div>
                    <label className="text-[10px] font-black uppercase tracking-widest text-cocoa-900/30 ml-1">
                      Canteen Name
                    </label>
                    <input
                      type="text"
                  ...
                  <div>
                    ...
                    <div className="mt-1.5 w-full rounded-[1rem] bg-sand-50 px-4 py-3.5 text-sm font-semibold text-cocoa-900/50 border border-sand-200 flex items-center gap-2">
                      <MapPin size={14} className="text-cocoa-900/30" />
                      MEDICAPS UNIVERSITY
                    </div>
                    <p className="mt-1.5 text-[9px] font-semibold text-cocoa-900/30 ml-1">
                      College is fixed and cannot be changed
                    </p>
                  </div>
                </div>
                <button
                  type="submit"
                  className="w-full flex items-center justify-center gap-2 rounded-[1rem] bg-[#1A1A1A] py-4 text-sm font-black text-white hover:bg-black transition-all active:scale-[0.98] shadow-lg shadow-black/10"
                >
                  <Save size={16} />
                  Save Changes
                </button>
              ...
              >
                ...
                <div className="bg-white rounded-[1.5rem] p-2 shadow-sm border border-sand-100">
                  <Link
                    to="/admin/help"
                    className="w-full flex items-center justify-between p-4 rounded-[1.25rem] hover:bg-sand-50 transition-colors group"
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="text-left">
                        <p className="text-sm font-black text-cocoa-900">
                          Help Centre
                        </p>
                        <p className="text-[10px] font-semibold text-cocoa-900/40 uppercase tracking-widest">
                          Support & Guides
                        </p>
                      </div>
                    </div>
