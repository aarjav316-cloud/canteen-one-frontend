...
import { 
  ArrowLeft, X, Package, Clock, User, 
  ChevronRight, ClipboardList, Search,
  CheckCircle2, XCircle, Flame, Gift
} from "lucide-react";
...
const DUMMY_ORDERS = [
  {
    ...
    items: [{ name: "Paneer Paratha", quantity: 2 }, { name: "Cold Coffee", quantity: 1 }],
    user: { name: "Abhishek G" },
    pickupCode: "AS89"
  },
  {
    _id: "ORD-9922",
    ...
    items: [{ name: "Samosa (2pcs)", quantity: 1 }, { name: "Chai", quantity: 1 }],
    user: { name: "Rahul S" },
    pickupCode: "ZX12"
  },
  {
    ...
    items: [{ name: "Butter Masala", quantity: 1 }, { name: "Rotis", quantity: 4 }],
    user: { name: "Simran K" },
    pickupCode: "KP44"
  }
];
    ...
    <div className="min-h-screen bg-[#F5F5F5] pb-24 font-sans text-cocoa-900">
      ...
      <div className="bg-[#F5F5F5] flex items-center justify-between pt-6 px-4 pb-3 border-b border-sand-200/60">
         <div className="flex items-center gap-3">
            ...
            <div>
               <h1 className="text-xl font-display font-black text-cocoa-900 tracking-tight leading-tight">Order Hub</h1>
      ...
      <div className="sticky top-0 z-50 bg-[#F5F5F5]/90 backdrop-blur-md pt-3 border-b border-sand-200/40">
        ...
        <div className="flex px-4 pb-2">
           <div className="flex w-full bg-sand-100/50 p-1 rounded-full border border-sand-100">
              {[
                { id: 'incoming', label: 'Accept', icon: Package, count: sections.incoming.length },
                { id: 'kitchen', label: 'Preparing', icon: Flame, count: sections.kitchen.length },
                { id: 'pickup', label: 'Pickup', icon: Gift, count: sections.pickup.length }
              ].map((tab) => (
                <button
                  key={tab.id}
                  ...
                  className={`relative flex flex-1 items-center justify-center gap-1.5 py-2.5 rounded-full text-[9px] font-black uppercase tracking-tighter transition-all ${
                    activeTab === tab.id ? 'bg-[#1A1A1A] text-white shadow-lg' : 'text-cocoa-900/40'
                  }`}
                >
                  <tab.icon size={12} />
                  {tab.label}
                  {tab.count > 0 && (
                    <span className={`px-1.5 py-0.5 rounded-md text-[7px] ${activeTab === tab.id ? 'bg-white text-black' : 'bg-sand-200 text-cocoa-900'}`}>
                      {tab.count}
                    </span>
                  )}
                </button>
      ...
      <main className="mx-auto w-full max-w-xl px-4 mt-6">
        <AnimatePresence>
          {notification && (
            <motion.div initial={{ y: -50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -50, opacity: 0 }} className="mb-4 bg-[#1A1A1A] p-4 rounded-[1rem] shadow-xl flex items-center justify-between text-white">
                ...
                <div className="flex items-center justify-between mb-3">
                   <div>
                      <div className="flex items-center gap-1.5">
                         <span className="text-[10px] font-black uppercase text-cocoa-900">ID #{order._id.slice(-4)}</span>
                         <span className="text-[8px] font-bold text-cocoa-900/30 flex items-center gap-1"><Clock size={9}/> {new Date(order.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                      </div>
                      <p className="mt-1 text-[11px] font-black text-rose-500 uppercase tracking-widest">{order.user?.name || "Guest"}</p>
                   </div>
                   <div className="text-right">
                      <p className="text-base font-display font-black text-cocoa-900 leading-none">{formatINR(order.totalAmount)}</p>
                      <p className="text-[8px] font-black text-cocoa-900/30 uppercase tracking-widest mt-0.5">Payable</p>
                   </div>
                </div>
                ...
                <div className="bg-sand-50/50 rounded-[1rem] p-3 space-y-1.5 border border-sand-50">
                   {order.items.map((item, idx) => (
                     <div key={idx} className="flex items-center justify-between text-[11px] font-semibold">
                        <span className="text-cocoa-900/40"><span className="text-cocoa-900 font-black">{item.quantity}x</span> {item.name}</span>
