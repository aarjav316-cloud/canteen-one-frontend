...
import { useWalletRefundListener } from "../hooks/useWalletRefundListener";
...
export default function StudentLayout() {
  ...
  // Listen for wallet refund events globally
  useWalletRefundListener();
    ...
    <div className="min-h-screen bg-luxury">
      {!hideChrome && (
        <>
          <RoleHeader
            title="Student Hub"
            subtitle="Fast ordering with real-time pickup tracking"
          />
          <RoleTabs items={studentNav} />
        </>
      )}
      <main
        className={`mx-auto w-full max-w-7xl ${hideChrome ? "pb-10" : "px-4 pb-10 md:px-8"}`}
      >
        <Outlet />
      </main>
    </div>
